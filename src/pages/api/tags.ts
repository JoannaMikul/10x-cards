...
const TAG_EVENT_SCOPE = "api/tags";

interface RawTagQueryParams {
  search?: string;
  limit?: string;
  cursor?: string;
  sort?: string;
...
export const GET: APIRoute = async ({ locals, url }) => {
  ...
  const validationResult = tagsQuerySchema.safeParse(rawQuery);
  ...
  let query: TagsQuery;
  try {
    query = buildTagsQuery(validationResult.data);
  } catch (error) {
    if (error instanceof InvalidTagCursorError) {
  ...
  try {
    const result = await listTags(supabase, query);
    ...
    if (isPostgrestError(error)) {
      const descriptor = buildErrorResponse(500, TAG_ERROR_CODES.DB_ERROR, "Failed to query tags from the database.", {
        code: error.code,
...
function invalidQueryResponse(
  message: string,
  rawQuery: RawTagQueryParams,
  details?: Record<string, unknown>
): Response {
  const descriptor = buildErrorResponse(400, TAG_ERROR_CODES.INVALID_QUERY, message);
  recordTagsEvent({
    severity: "info",
    status: descriptor.status,
    code: TAG_ERROR_CODES.INVALID_QUERY,
...
function serializeStructuredQuery(query: TagsQuery): Record<string, string | number | undefined> {
  return {
    search: query.search,
    limit: query.limit,
    cursor: query.cursor,
...
function recordTagsEvent(payload: TagEventPayload): void {
  const entry = {
    scope: TAG_EVENT_SCOPE,
  ...
  logger(`[${TAG_EVENT_SCOPE}]`, JSON.stringify(entry));
}
